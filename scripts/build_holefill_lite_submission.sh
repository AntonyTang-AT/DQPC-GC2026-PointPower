#!/usr/bin/env bash
# Build Enhancement Only submission: ft PD-LTS + SuperPC CG-hole hybrid (holefill lite).
set -euo pipefail
...
echo "[build_holefill_lite] ENH_DIR=$ENH_DIR"
...
print("[build_holefill_lite] gate:", cfg.name)
PY

if [[ -f "$FINETUNE_CKPT_SRC" ]]; then
  cp -f "$FINETUNE_CKPT_SRC" "${ENH_DIR}/models/DenoiseFlow-light-UVG-finetune.ckpt"
  echo "[build_holefill_lite] bundled finetune ckpt"
else
  echo "[build_holefill_lite] WARN: no finetune ckpt at FINETUNE_CKPT_SRC" >&2
fi
...
#!/usr/bin/env bash
# Enhancement Only: CG -> ft PD-LTS -> SuperPC blend_cg -> holefill lite hybrid refine.
set -euo pipefail
SRC_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "${SRC_DIR}/common.sh"

export OUT_DIR="${OUT_DIR:-${GC2026_ROOT}/output/submission_candidate_holefill_lite}"
...
echo "[run.sh] Stage3 holefill lite hybrid -> $OUT_DIR"
...
OUT_DIR="${OUT_DIR:-${GC2026_ROOT}/output/submission_smoke_holefill_lite}"
...
SRC_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "${SRC_DIR}/common.sh"
OUT="${OUT_DIR:-${GC2026_ROOT}/output/submission_candidate_holefill_lite}"
...
"$PYTHON" "${SRC_DIR}/make_submission.py" \
  ...
  --title "UVG-CWI-DQPC GC2026 Enhancement Only holefill lite hybrid" \
  --post-processing "$GATE_JSON" --cg-version "${UVG_CG_VERSION:-v2}" \
  --cg-source "official" \
  --data-split "official_val=TrumanShow,VictoryHeart,VirtualLife" \
  --pipeline-notes "CGv2 -> UVG-finetuned PD-LTS light -> SuperPC blend_cg secondary -> holefill lite (primary anchor, CG-hole mask, max10pct fill)"
if [[ -f "$VAL_PAIRS" ]]; then
...
# Environment setup (holefill lite hybrid)
...
Preset: `holefill_lite_fill0.25_max10pct_adaptive_post25` in `config/gate_config.json`.
EOF

cat > "${ENH_DIR}/README.md" <<'EOF'
# GC2026 Team — Enhancement Only (holefill lite hybrid)

## Algorithm Name
UVG-finetuned PD-LTS light + SuperPC CG-hole fill lite (`holefill_lite_fill0.25_max10pct_adaptive_post25`)

## Algorithm Description
Official CG PLY (v2) → **PD-LTS light UVG finetune** (primary geometry) → **SuperPC** `blend_cg` (secondary, hole fill only) → hybrid refine: primary anchor, CG-hole mask, fill 0.25 mm, max 10% SuperPC points, adaptive post-SOR. KNN color from CG.
...
Output: `$GC2026_ROOT/output/submission_candidate_holefill_lite/` (2155 ENH PLY).
...
| **holefill lite (this package)** | **15.128** |
| CG baseline | 17.552 |
...
echo "[build_holefill_lite] DONE $ENH_DIR"
