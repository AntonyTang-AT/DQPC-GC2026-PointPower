...
python "${GC2026_ROOT}/scripts/write_runtime_summary.py" \
  --out-dir "$OUT" \
...
if [[ "$PACK_TAR" == "1" ]]; then
  ...
  bash "${GC2026_ROOT}/scripts/pack_submission.sh" "$PACK_SRC" || true
fi

python "${GC2026_ROOT}/scripts/generate_status_report.py" 2>/dev/null || true
