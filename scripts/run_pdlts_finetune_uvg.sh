...
#
#   bash scripts/setup_pdlts_train.sh          # once
#   bash scripts/run_pdlts_finetune_uvg.sh smoke
#   GPUS=4 bash scripts/run_pdlts_finetune_uvg.sh train
set -euo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
MODE="${1:-smoke}"
GPUS="${GPUS:-4}"
OUT="${OUT_DIR:-$ROOT/output/pdlts_finetune_uvg}"
PAIRS="${PAIRS_FILE:-$ROOT/data/processed/train_pairs_official_cgv2.txt}"
LOG="$OUT/logs"
mkdir -p "$LOG"
...
smoke() {
  ...
  python "$ROOT/scripts/run_pdlts_finetune_uvg.py" \
    --pairs-file "$PAIRS" \
    --out-dir "$OUT/smoke" \
    --max-train-frames 16 \
